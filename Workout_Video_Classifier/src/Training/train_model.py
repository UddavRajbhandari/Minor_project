...
class WorkoutModelTrainer:
    ...
    def build_model(self, input_shape, num_classes):
        model = tf.keras.Sequential([
            ...
            Conv1D(256, kernel_size=3, padding='same', activation='relu',
                kernel_regularizer=tf.keras.regularizers.l2(0.01)),
            BatchNormalization(),
            # Global pooling to summarize feature maps over the time dimension
        ...
        # Print model summary with the new input shape
        print("\nModel Summary:")
    ...
    def save_training_plots(self, fold_histories, save_path):
        ...
        plt.title('Model Accuracy Across Folds ( CNN with Weighted Features)')
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy')
        ...
        plt.title('Model Loss Across Folds ( CNN with Weighted Features)')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.grid(True)
    ...
    def save_fold_results(self, fold_results, fold_histories, save_path):
        ...
        for fold, (result, history) in enumerate(zip(fold_results, fold_histories)):
            ...
            plt.title(f'Model Accuracy - Fold {fold+1} ( CNN with Weighted Features)')
            plt.xlabel('Epoch')
            plt.ylabel('Accuracy')
            plt.legend()
            plt.grid(True)
            ...
            plt.title(f'Model Loss - Fold {fold+1} ( CNN with Weighted Features)')
            plt.xlabel('Epoch')
            plt.ylabel('Loss')
            plt.legend()
            plt.grid(True)
            ...
            plt.title(f'Confusion Matrix - Fold {fold+1} ( CNN with Weighted Features)')
        ...
        print("\nK-Fold Cross Validation Results ( CNN with Weighted Features):")
    ...
    def train_with_kfold(self, epochs=100, batch_size=16, n_splits=5, save_path=None):
        ...
        print(f"\nStarting {n_splits}-fold stratified cross validation with  CNN architecture...")
...
if __name__ == "__main__":
    ...
    save_path = r"D:\minor\test_k_fold_CNN_landmark"
